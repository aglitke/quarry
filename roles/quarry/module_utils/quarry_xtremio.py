...
class XtremIOAlreadyMappedError(Exception):
    pass


class XtremIOSnapshotsLimitExceeded(Exception):
    pass
...
class XtremIOClient(object):
    def __init__(self, configuration, cluster_id):
        ...
        self.verify = (self.configuration.get('driver_ssl_cert_verify') or
                       False)
        if self.verify:
            verify_path = (self.configuration.get('driver_ssl_cert_path') or
                           None)
            if verify_path:
    ...
    @quarry_common.retry(XtremIOArrayBusy,
                         CONFIG_DEFAULTS['xtremio_array_busy_retry_count'],
                         CONFIG_DEFAULTS['xtremio_array_busy_retry_interval'])
    def req(self, object_type='volumes', method='GET', data=None,
            name=None, idx=None, ver='v1'):
        if not data:
            data = {}
        if name and idx:
            msg = "can't handle both name and index in req"
            logging.error(msg)
            raise quarry_common.VolumeDriverException(msg)
        ...
        logging.debug('%(type)s %(url)s', {'type': method, 'url': url})
        try:
            ...
            msg = ('Exception: %s') % six.text_type(exc)
            raise quarry_common.VolumeDriverException(msg)
    ...
    def handle_errors(self, response, key, object_type):
        if response.status_code == 400:
            ...
            if err_msg.endswith(OBJ_NOT_FOUND_ERR):
                logging.warning("object %(key)s of type %(typ)s not found, "
                                "%(err_msg)s",
                                {'key': key,
                                 'typ': object_type,
                                 'err_msg': err_msg})
...
class XtremIOClient3(XtremIOClient):
    ...
    def find_lunmap(self, ig_name, vol_name):
        try:
            ...
            raise (quarry_common.VolumeDriverException
                   ("can't find lun-map, ig:%(ig)s vol:%(vol)s" %
    ...
    def get_initiator(self, port_address):
        try:
            res = self.req('initiators', 'GET', name=port_address)
            logging.debug('get_initiator: port_address = %s, res = %s', port_address, res)
            return res['content']
...
class XtremIOClient4(XtremIOClient):
    ...
    def find_lunmap(self, ig_name, vol_name):
        try:
            ...
            raise quarry_common.VolumeNotFound(vol_name)
    ...
    def create_snapshot(self, src, dest, ro=False):
        ...
        except quarry_common.VolumeBackendAPIException:
            # reverting
            msg = 'Failed to rename the created snapshot, reverting.'
            logging.error(msg)
...
class Driver(quarry_common.Driver):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        self.protocol = None
        self.backend_name = (self.configuration.get('volume_backend_name')
                             or self.driver_name)
        self.cluster_id = (self.configuration.get('xtremio_cluster_name')
                           or '')
        self.provisioning_factor = (self.configuration.
                                    get('max_over_subscription_ratio')
    ...
    def get_volume(self, volume):
        try:
            vol_obj = self.client.req('volumes', name=volume.id)['content']
            size = int(math.ceil(float(vol_obj['vol-size']) /
                                 quarry_common.MB))
    ...
    def check_for_setup_error(self):
        ...
        if ver < self.MIN_XMS_VERSION:
            msg = ('Invalid XtremIO version %(cur)s, version %(min)s or up '
                   'is required') % {'min': self.MIN_XMS_VERSION, 'cur': ver}
            logging.error(msg)
            raise quarry_common.VolumeBackendAPIException(msg)
        else:
    ...
    def create_cloned_volume(self, volume, src_vref):
        ...
        #                                                    src_vref['id'])
        # limit = self.configuration.get('xtremio_volumes_per_glance_cache')
    ...
    def delete_volume(self, volume):
        ...
        try:
            ...
            logging.info("volume %s doesn't exist", volume.id)
    ...
    def delete_snapshot(self, snapshot):
        ...
        try:
            ...
            logging.info("snapshot %s doesn't exist", snapshot.id)
    ...
    def update_migrated_volume(self, ctxt, volume, new_volume,
        ...
        try:
            ...
            logging.error('Unable to rename the logical volume for volume: %s',
                          original_name)
    ...
    def _update_volume_stats(self):
        ...
        physical_space = int(sys["ud-ssd-space"]) / quarry_common.MB
        used_physical_space = int(sys["ud-ssd-space-in-use"]) / quarry_common.MB
        free_physical = physical_space - used_physical_space
        actual_prov = int(sys["vol-size"]) / quarry_common.MB
        ...
        try:
            ...
            if (
                    ...
                    (not vol_obj['ancestor-vol-id'] or
                             ...
                             volume.volume_id)):
                msg = json.dumps({'existing_ref': lv_name,
                                  'reason': 'Not a snapshot of vol %s' %
                                  volume.volume_id})
                raise ManageExistingInvalidReference(msg)
        except NotFound:
            msg = json.dumps(
    ...
    def manage_existing_get_size(self, volume, existing_ref,
        ...
        lv_size = int(math.ceil(float(vol_obj['vol-size']) / quarry_common.MB))
    ...
    def unmanage(self, volume, is_snapshot=False):
        ...
        try:
            ...
            logging.info("%(typ)s with the name %(name)s wasn't found, "
                         "can't unmanage" %
                     {'typ': 'Snapshot' if is_snapshot else 'Volume',
                      'name': volume['id']})
            raise quarry_common.VolumeNotFound(volume['id'])
    ...
    def extend_volume(self, volume, new_size):
        ...
        try:
            ...
            msg = "can't find the volume to extend"
            raise quarry_common.VolumeDriverException(msg)
    ...
    def terminate_connection(self, volume, connector, **kwargs):
        ...
        for ig_idx in self._get_ig_indexes_from_initiators(connector):
            ...
            logging.debug('Removing lun map %s.', lm_name)
            try:
                self.client.req('lun-maps', 'DELETE', name=lm_name)
            except NotFound:
                logging.warning("terminate_connection: lun map not found")
    ...
    def create_consistencygroup(self, context, group):
        ...
        self.client.req('consistency-groups', 'POST', data=create_data,
                        ver='v2')
        return {'status': quarry_common.ConsistencyGroupStatus.AVAILABLE}
    ...
    def create_consistencygroup_from_src(self, context, group, volumes,
        ...
        if not (cgsnapshot and snapshots and not source_cg or
            ...
            raise quarry_common.InvalidInput(
                "create_consistencygroup_from_src only supports a "
...
class ISCSIDriver(Driver):
    ...
    def __init__(self, config, *args, **kwargs):
        super(ISCSIDriver, self).__init__(config, *args, **kwargs)
        self.protocol = 'iSCSI'
    ...
    def _get_iscsi_properties(self, lunmap):
        ...
        if not portals:
            msg = "XtremIO not configured correctly, no iscsi portals found"
            logging.error(msg)
            raise quarry_common.VolumeDriverException(msg)
